...
parameters{
  ...
  // real<lower=0> beta_dates_sd_sm;
   ...
   real<lower=0> phi_rp;
   real rho0_rp;
   vector[P_cities-1] rho_cities_raw_rp;
   real<lower=0> rho_cities_sd_rp;
   real rho_outbreak_rp;
}
...
transformed parameters{
 ...
 for(i in 1:N){
  ...
  vector[N] log_mu_rp; /// logarise this!
   vector[P_cities] rho_cities_rp = sum2zero_effect(
   P_cities, rho_cities_raw_rp, 1
 );
  vector[N] rho_rp;
  rho_rp = inv_logit(rho0_rp + 
  X_cities * rho_cities_rp+
  X_outbreak_sm * rho_outbreak_rp);
...
model{
  ...
  // rho_rp ~ beta( 0.5, 0.5 );
  rho0_rp ~ normal( 0 , 3.5^2);
  rho_cities_raw_rp ~ normal(0,cities_s2z_sd_sm);
  // rho_cities_sd_rp ~ cauchy( 0 , 1);
  rho_outbreak_rp ~ normal( 0, 1.5 );
}
