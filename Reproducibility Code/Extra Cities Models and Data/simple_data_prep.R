...
citydate_interact <- matrix(nrow = nrow(onehot_city), ncol = ncol(onehot_city)*ncol(onehot_series)) # 7 cities * 2dates
col_idx <- 1
...
#          OUTBREAK = as.integer(POS_PROP>OUTBREAK_THRESH)) %>%
#   select(OUTBREAK)
outbreak_sm <- extra_df %>% mutate(N_PROP_POS = N_POSITIVE/N_TOTAL) %>%
  group_by(CITY) %>%
  mutate(MIN_POS = min(N_PROP_POS),
         MAX_POS = max(N_PROP_POS)) %>%
  mutate(OUTBREAK = as.integer(N_PROP_POS > 2*MIN_POS))
...
extra_data$X_citydate <- citydate_interact

saveRDS(extra_data, file.path(out.dir,"simple_inputdata.rds"))
saveRDS(extra_df, file.path(out.dir, "simple_data_key.rds"))
