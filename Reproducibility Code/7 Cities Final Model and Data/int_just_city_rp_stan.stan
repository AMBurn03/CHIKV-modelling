...
parameters{
  ...
  // vector[P_citydate - 1] rho_cd_raw_rp;
  // real<lower=0> rho_cd_sd_rp;
...
transformed parameters {
  ...
  //     vector[P_citydate] rho_cd_rp = sum2zero_effect(
  //   P_citydate, rho_cd_raw_rp, rho_cd_sd_rp);
    
  vector[N] log_mu_rp; 
  vector[N] logit_rho_rp;
  logit_rho_rp = rho_0_rp + X_cities * rho_city_rp;
    // X_age * rho_age_rp + X_citydate * rho_cd_rp;
...
model {
  ...
  // rho_cd_raw_rp ~ normal( 0 , cd_s2z_sd_sm);
  // rho_cd_sd_rp ~ cauchy( 0 , 1 );
}
